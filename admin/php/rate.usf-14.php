...
if(checkStates_14($shipment["o_state"],$shipment["d_state"]) && $FUEL_BACKOUT && $HTTP_HOST=="admin.trollcompany.com"){
	...
	if($classes && $weights){
		...
		if($fak_class_temp){
			...
			$usf_costs_temp = autoRateUSF_14($shipment["o_zip"],$shipment["d_zip"],$new_classes,$weights);
		...
		$usf_rates = autoRateUSF_14($shipment["o_zip"],$shipment["d_zip"],$classes,$weights);
		if(is_array($usf_rates)){
			...
			if($carrier_costs){
				...
				if($usf_cost){
					...
					$cost = $usf_cost - ($usf_cost * $discount_cost);
				}else{
...
// does a quote
function autoRateUSF_14($origin_zip,$destination_zip,$classes,$weights){
...
function checkStates_14($o_state,$d_state){
	$states = array("AK","WA","OR","CA","NV","ID","UT","MT","WY","CO");
